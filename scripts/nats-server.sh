...
cmd_start() {
    ...
    if [ "${NATS_MANAGER_DOMAIN}" != "localhost" ]; then
        echo "  HTTP:   http://${NATS_MANAGER_DOMAIN}:${NATS_HTTP_PORT}"
        echo "  NATS:   nats://${NATS_MANAGER_DOMAIN}:${NATS_PORT}"
        echo "  WSS:    wss://${NATS_MANAGER_DOMAIN}:${NATS_WS_PORT}"
    else
        echo "  HTTP:   http://localhost:${NATS_HTTP_PORT}"
        echo "  NATS:   nats://localhost:${NATS_PORT}"
        echo "  WSS:    wss://localhost:${NATS_WS_PORT}"
    fi
    echo ""
...
cmd_status() {
    ...
    if is_running "$PID_FILE"; then
        ...
        echo "  Config: $CONFIG_DIR/nats-server-runtime.conf"
        echo "  HTTP:   http://localhost:${NATS_HTTP_PORT}"
        echo "  Server: nats://localhost:${NATS_PORT}"
        
        # Try to fetch stats
        if curl -s "http://localhost:${NATS_HTTP_PORT}/varz" > /dev/null 2>&1; then
            local conns=$(curl -s "http://localhost:${NATS_HTTP_PORT}/connz" | grep -o '"num_connections":[0-9]*' | cut -d: -f2)
    ...
    if is_running "$UI_PID_FILE"; then
        ...
        if [ "${NATS_MANAGER_DOMAIN}" != "localhost" ]; then
            echo "  URL:    http://${NATS_MANAGER_DOMAIN}:${NATS_MANAGER_PORT}"
        else
            echo "  URL:    http://localhost:${NATS_MANAGER_PORT}"
        fi
    else
...
cmd_ui() {
    if is_running "$UI_PID_FILE"; then
        ...
        if [ "${NATS_MANAGER_DOMAIN}" != "localhost" ]; then
            echo "Access at: http://${NATS_MANAGER_DOMAIN}:${NATS_MANAGER_PORT}"
        else
            echo "Access at: http://localhost:${NATS_MANAGER_PORT}"
        fi
        return 0
    fi
...
cmd_manage() {
    ...
    if [ "${NATS_MANAGER_DOMAIN}" != "localhost" ]; then
        echo "NATS: http://${NATS_MANAGER_DOMAIN}:${NATS_HTTP_PORT}"
        echo "UI:   http://${NATS_MANAGER_DOMAIN}:${NATS_MANAGER_PORT}"
    else
        echo "NATS: http://localhost:${NATS_HTTP_PORT}"
        echo "UI:   http://localhost:${NATS_MANAGER_PORT}"
    fi
}
